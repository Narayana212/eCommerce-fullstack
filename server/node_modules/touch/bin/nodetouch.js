...
/**
 * Displays usage information for the 'touch' command and exits the process
 * @param {number|boolean} code - Exit code for the process. If truthy, logs to console.error, otherwise to console.log
...
process.exitCode = 0
/**
 * Executes the 'touch' operation on multiple files concurrently
 * @param {Array<string>} files - An array of file paths to be touched
 * @param {Object} options - Options to be passed to the touch operation
 * @returns {Promise<Array>} A promise that resolves to an array of results from touching each file
 */
Promise.all(files.map(f => touch(f, options)))
  /**
   * Catches an error and sets the process exit code to 1
   * @param {Error} er - The error object caught by the catch block
